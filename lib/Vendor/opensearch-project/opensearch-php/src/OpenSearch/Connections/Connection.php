...
use Exception;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Ring\Core;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Ring\Exception\ConnectException;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Ring\Exception\RingException;
...
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\Conflict409Exception;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\Curl\CouldNotConnectToHost;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\Curl\CouldNotResolveHostException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\Curl\OperationTimeoutException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\Forbidden403Exception;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\MaxRetriesException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\Missing404Exception;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\NoDocumentsToGetException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\NoShardAvailableException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\OpenSearchException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\RequestTimeout408Exception;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\RoutingMissingException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\ScriptLangNotSupportedException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\ServerErrorResponseException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\TransportException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\Unauthorized401Exception;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Serializers\SerializerInterface;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Transport;
...
class Connection implements ConnectionInterface
{
     ...
     */
    public function __construct(callable $handler, array $hostDetails, array $connectionParams, SerializerInterface $serializer, LoggerInterface $log, LoggerInterface $trace)
    {
        if (isset($hostDetails['port']) !== \true) {
        ...
        if (isset($connectionParams['client']['headers']['Authorization']) === \false && isset($connectionParams['client']['curl'][\CURLOPT_HTTPAUTH]) === \false && isset($hostDetails['user']) && isset($hostDetails['pass'])) {
        ...
        $this->headers['User-Agent'] = [sprintf('opensearch-php/%s (%s %s; PHP %s)', Client::VERSION, \PHP_OS, $this->getOSVersion(), \PHP_VERSION)];
        $host = $hostDetails['host'];
        $path = null;
        if (isset($hostDetails['path']) === \true) {
    ...
    public function performRequest(string $method, string $uri, ?array $params = [], $body = null, array $options = [], ?Transport $transport = null)
    {
        ...
        $request = ['http_method' => $method, 'scheme' => $this->transportSchema, 'uri' => $this->getURI($uri, $params), 'body' => $body, 'headers' => array_merge(['Host' => [$host]], $headers)];
    ...
    private function wrapHandler(callable $handler): callable
    {
        return function (array $request, Connection $connection, ?Transport $transport, $options) use ($handler) {
            ...
            $response = Core::proxy($handler($request), function ($response) use ($connection, $transport, $request, $options) {
                ...
                return isset($request['client']['verbose']) && $request['client']['verbose'] === \true ? $response : $response['body'];
            });
            return $response;
        };
    }
    ...
    {
        if (isset($params) === \true && !empty($params)) {
    ...
    public function logRequestSuccess(array $request, array $response): void
    {
        ...
        $this->trace->debug('Response:', array('response' => $response['body'], 'method' => $request['http_method'], 'uri' => $uri, 'port' => $port, 'HTTP code' => $response['status'], 'duration' => $response['transfer_stats']['total_time']));
    }
    /**
    ...
    public function logRequestFail(array $request, array $response, \Throwable $exception): void
    {
        ...
        $uri = $this->addPortInUrl($response['effective_url'], (int) $port);
        $this->log->debug('Request Body', array($request['body']));
        $this->log->warning('Request Failure:', array('method' => $request['http_method'], 'uri' => $uri, 'port' => $port, 'headers' => $request['headers'], 'HTTP code' => $response['status'], 'duration' => $response['transfer_stats']['total_time'], 'error' => $exception->getMessage()));
        $this->log->warning('Response', array($response['body']));
        // Build the curl command for Trace.
        $curlCommand = $this->buildCurlCommand($request['http_method'], $uri, $request['body']);
        $this->trace->info($curlCommand);
        $this->trace->debug('Response:', array('response' => $response, 'method' => $request['http_method'], 'uri' => $uri, 'port' => $port, 'HTTP code' => $response['status'], 'duration' => $response['transfer_stats']['total_time']));
    }
    public function ping(): bool
    {
        $options = ['client' => ['timeout' => $this->pingTimeout, 'never_retry' => \true, 'verbose' => \true]];
        try {
    ...
    {
        $options = ['client' => ['timeout' => $this->pingTimeout, 'never_retry' => \true]];
    ...
    private function getOSVersion(): string
    {
        if ($this->OSVersion === null) {
            $this->OSVersion = strpos(strtolower(ini_get('disable_functions')), 'php_uname') !== \false ? '' : php_uname("r");
        }
    ...
    private function process5xxError(array $request, array $response, array $ignore): void
    {
        ...
        if ($statusCode === 500 && strpos($responseBody, "RoutingMissingException") !== \false) {
            ...
            $exception = new ServerErrorResponseException($this->convertBodyToString($responseBody, $statusCode, $exception));
        }
        $this->logRequestFail($request, $response, $exception);
        throw $exception;
    }
    ...
    {
        if (empty($body)) {
            return sprintf("Unknown %d error from OpenSearch %s", $statusCode, $exception->getMessage());
        }
    ...
    private function tryDeserialize400Error(array $response): OpenSearchException
    {
        return $this->tryDeserializeError($response, BadRequest400Exception::class);
    }
    private function tryDeserialize500Error(array $response): OpenSearchException
    {
    ...
    private function tryDeserializeError(array $response, string $errorClass): OpenSearchException
    {
        ...
        if (is_array($error) === \true) {
            ...
            if (is_array($error['error']) && array_key_exists('reason', $error['error']) === \true) {
