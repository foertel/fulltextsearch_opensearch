...
use OCA\FullTextSearch_OpenSearch\Vendor\Aws\Credentials\CredentialProvider;
use OCA\FullTextSearch_OpenSearch\Vendor\Aws\Credentials\Credentials;
use OCA\FullTextSearch_OpenSearch\Vendor\Aws\Credentials\CredentialsInterface;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Ring\Client\CurlHandler;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Ring\Client\CurlMultiHandler;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Ring\Client\Middleware;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\AuthenticationConfigException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\InvalidArgumentException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Common\Exceptions\RuntimeException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\ConnectionPool\AbstractConnectionPool;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\ConnectionPool\Selectors\RoundRobinSelector;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\ConnectionPool\Selectors\SelectorInterface;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\ConnectionPool\StaticNoPingConnectionPool;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Connections\ConnectionFactory;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Connections\ConnectionFactoryInterface;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Connections\ConnectionInterface;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Handlers\SigV4Handler;
...
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Serializers\SmartSerializer;
use OCA\FullTextSearch_OpenSearch\Vendor\Psr\Log\LoggerInterface;
use OCA\FullTextSearch_OpenSearch\Vendor\Psr\Log\NullLogger;
use ReflectionClass;
...
class ClientBuilder
{
     ...
     * @var array
     */
    private $connectionPoolArgs = ['randomizeHosts' => \true];
    /**
     * @var array|null
     */
    private $hosts;
    ...
    public static function fromConfig(array $config, bool $quiet = \false): Client
    {
        ...
        foreach ($config as $key => $value) {
            $method = in_array($key, self::ALLOWED_METHODS_FROM_CONFIG, \true) ? $key : "set{$key}";
     ...
     */
    public function setSSLVerification($value = \true): ClientBuilder
    {
    ...
    public function build(): Client
    {
        ...
        if (!is_null($this->basicAuthentication)) {
            ...
            $this->connectionParams['client']['curl'] += [\CURLOPT_HTTPAUTH => \CURLAUTH_BASIC, \CURLOPT_USERPWD => $this->basicAuthentication];
        }
    ...
    private function buildTransport(): void
    {
        ...
        if (is_string($this->connectionPool)) {
            $this->connectionPool = new $this->connectionPool($connections, $this->selector, $this->connectionFactory, $this->connectionPoolArgs);
        }
    ...
    private function buildConnectionsFromHosts(array $hosts): array
    {
        ...
        foreach ($hosts as $host) {
            if (is_string($host)) {
                ...
                $this->logger->error("Could not parse host: " . print_r($host, \true));
                throw new RuntimeException("Could not parse host: " . print_r($host, \true));
            }
    ...
    private function normalizeExtendedHost(array $host): array
    {
        if (isset($host['host']) === \false) {
            $this->logger->error("Required 'host' was not defined in extended format: " . print_r($host, \true));
            throw new RuntimeException("Required 'host' was not defined in extended format: " . print_r($host, \true));
        }
    ...
    private function normalizeCredentialProvider($provider): ?callable
    {
        ...
        if ($provider instanceof CredentialsInterface) {
            ...
            return CredentialProvider::fromCredentials(new Credentials($provider['key'], $provider['secret'], isset($provider['token']) ? $provider['token'] : null, isset($provider['expires']) ? $provider['expires'] : null));
        }
