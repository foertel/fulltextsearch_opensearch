...
namespace OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Aws;
...
use OCA\FullTextSearch_OpenSearch\Vendor\Aws\Exception\CredentialsException;
use OCA\FullTextSearch_OpenSearch\Vendor\Aws\Signature\SignatureInterface;
...
class SigningClientFactory
{
    ...
    public function __construct(protected ?SignatureInterface $signer = null, protected ?CredentialProvider $provider = null, protected ?LoggerInterface $logger = null)
    {
    }
    /**
    ...
    protected function getCredentialProvider(array $options): CredentialProvider|\Closure|null|callable
    {
        ...
        if (isset($options['credentials'])) {
            return CredentialProvider::fromCredentials(new Credentials($options['credentials']['access_key'] ?? '', $options['credentials']['secret_key'] ?? '', $options['credentials']['session_token'] ?? null));
        }
    ...
    protected function getSigner(array $options): SignatureInterface
    {
        ...
        if (!in_array($service, self::ALLOWED_SERVICES, \true)) {
