...
namespace OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Handlers;

use OCA\FullTextSearch_OpenSearch\Vendor\Aws\Credentials\CredentialProvider;
use OCA\FullTextSearch_OpenSearch\Vendor\Aws\Signature\SignatureV4;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Psr7\Request;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Psr7\Uri;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Psr7\Utils;
...
class SigV4Handler
{
     ...
     */
    public function __construct(string $region, string $service, ?callable $credentialProvider = null, ?callable $wrappedHandler = null)
    {
    ...
    public function __invoke(array $request)
    {
        ...
        $signedRequest = $this->signer->signRequest($psr7Request, $creds);
    ...
    private function createPsr7Request(array $ringPhpRequest): Request
    {
        ...
        $uri = (new Uri($ringPhpRequest['uri']))->withScheme($ringPhpRequest['scheme'])->withHost($ringPhpRequest['headers'][$hostKey][0]);
        if (isset($ringPhpRequest['query_string'])) {
            $uri = $uri->withQuery($ringPhpRequest['query_string']);
        }
        // Create a PSR-7 request from the array passed to the handler
        return new Request($ringPhpRequest['http_method'], $uri, $ringPhpRequest['headers'], $ringPhpRequest['body']);
    }
    /**
    ...
    private function createRingRequest(RequestInterface $request, array $originalRequest): array
    {
        ...
        $ringRequest = ['http_method' => $request->getMethod(), 'scheme' => $uri->getScheme(), 'uri' => $uri->getPath(), 'body' => $body, 'headers' => $request->getHeaders(), 'client' => $client];
        if ($uri->getQuery()) {
