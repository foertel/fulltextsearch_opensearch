...
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Ring\Future\FutureArrayInterface;
...
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Exception\NotFoundHttpException;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Transport;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\TransportInterface;
abstract class BooleanRequestWrapper
{
    ...
    {
        try {
            $transport->sendRequest($endpoint->getMethod(), $endpoint->getURI(), $endpoint->getParams(), $endpoint->getBody(), $endpoint->getOptions());
    ...
    public static function performRequest(AbstractEndpoint $endpoint, Transport $transport)
    {
        ...
        try {
            $response = $transport->performRequest($endpoint->getMethod(), $endpoint->getURI(), $endpoint->getParams(), $endpoint->getBody(), $endpoint->getOptions());
