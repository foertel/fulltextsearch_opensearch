 ...
 */
namespace OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Endpoints;

use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\EndpointInterface;
use OCA\FullTextSearch_OpenSearch\Vendor\OpenSearch\Exception\UnexpectedValueException;
...
abstract class AbstractEndpoint implements EndpointInterface
{
    ...
    private function checkUserParams(array $params)
    {
        ...
        $whitelist = array_merge($this->getParamWhitelist(), ['pretty', 'human', 'error_trace', 'source', 'filter_path', 'opaqueId']);
    ...
    private function extractOptions(&$params)
    {
        ...
        if (isset($params['client']) === \true) {
            // Check if the opaqueId is populated and add the header
            if (isset($params['client']['opaqueId']) === \true) {
    ...
    private function convertArraysToStrings(array $params): array
    {
        foreach ($params as $key => &$value) {
            if (!($key === 'client' || $key == 'custom') && is_array($value) === \true) {
                if ($this->isNestedArray($value) !== \true) {
                    $value = implode(",", $value);
                }
            }
        }
