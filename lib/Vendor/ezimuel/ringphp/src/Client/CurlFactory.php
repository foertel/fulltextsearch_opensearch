...
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Stream\LazyOpenStream;
...
class CurlFactory
{
    ...
    private static function createErrorResponse(callable $handler, array $request, array $response)
    {
        ...
        $error = isset($response['curl']['errno']) && isset($connectionErrors[$response['curl']['errno']]) ? new ConnectException($message) : new RingException($message);
