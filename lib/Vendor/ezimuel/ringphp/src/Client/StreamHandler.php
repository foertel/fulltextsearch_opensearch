...
namespace OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Ring\Client;
...
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Ring\Future\CompletedFutureArray;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Stream\InflateStream;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Stream\StreamInterface;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Stream\Stream;
use OCA\FullTextSearch_OpenSearch\Vendor\GuzzleHttp\Stream\Utils;
/**
...
class StreamHandler
{
    ...
    private function createResponse(array $request, $url, $stream)
    {
        ...
        $response = ['version' => substr($parts[0], 5), 'status' => $parts[1], 'reason' => isset($parts[2]) ? $parts[2] : null, 'headers' => Core::headersFromLines($hdrs), 'effective_url' => $url];
    ...
    private function checkDecode(array $request, array $response, $stream)
    {
        ...
        if (!empty($request['client']['decode_content'])) {
            switch (Core::firstHeader($response, 'Content-Encoding', \true)) {
                case 'gzip':
                case 'deflate':
    ...
    private function createErrorResponse($url, RingException $e)
    {
        ...
        return new CompletedFutureArray(['status' => null, 'body' => null, 'headers' => [], 'effective_url' => $url, 'error' => $e]);
    }
    /**
    ...
    private function createStream($url, array $request)
    {
        ...
        if ((!isset($request['version']) || $request['version'] == '1.1') && !Core::hasHeader($request, 'Connection')) {
